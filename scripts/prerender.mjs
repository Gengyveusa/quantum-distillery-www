...
// Routes to prerender. Note that src/main.tsx uses HashRouter, so /sim and
// /instructor live at /#/sim and /#/instructor — hash fragments never reach the
// server and cannot be prerendered as separate documents. Giving them their own
// indexable URLs requires switching to BrowserRouter first.
const ROUTES = ['/'];
...
try {
  for (const route of ROUTES) {
    ...
    if (wordCount < 100) {
      throw new Error(
        `${route} rendered only ${wordCount} words (expected 100+). ` +
